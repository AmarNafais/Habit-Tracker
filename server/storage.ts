...
export class DatabaseStorage implements IStorage {
    ...
    async createHabit(insertHabit: InsertHabit): Promise<Habit> {
        await db.insert(habits).values({ ...insertHabit, completedDays: [] });

        const [habit] = await db
            .select()
            .from(habits)
            .where(eq(habits.name, insertHabit.name));
        if (!habit) throw new Error('Habit not found after insert');
    ...
    async toggleHabitCompletion(id: number, date: string): Promise<Habit> {
        ...
        await db.update(habits).set({ completedDays }).where(eq(habits.id, id));
